...
## 💎 프리미엄 고급 기능 (Premium Features)

1. **Zero-Knowledge 종단간 암호화 (E2EE):** 마스터 비밀번호 기반 `PBKDF2` 솔트 처리를 거쳐 `AES-GCM-256` 대칭키를 유도하며, 일지 본문과 CBT 텍스트가 브라우저를 벗어나기 전에 로컬 단에서 암호화됩니다. DB 유출 시에도 안전합니다.
2. **마음챙김 호흡 페이서 & 앰비언트 자연음 신디사이저:** 4-7-8 호흡 유도 SVG 맥동 서클 가이드 및 Web Audio API를 활용하여 외부 다운로드 없이 잔잔한 빗소리(Pink Noise)와 티베트 싱잉볼 공명음(C#3 정현파 중첩)을 브라우저가 실시간 합성/재생합니다.
3. **SVG 마음 건강 상세 분석 통계 보고서:** 최근 15회 기분 변화 그래프(SVG Line Chart) 및 CBT 일지 내 인지 왜곡 어근 분류 탐지 통계(SVG 도넛 파이 차트)를 라이브러리 없이 네이티브 컴포넌트로 세련되게 시각화해 줍니다.
4. **오프라인 PWA 및 자동 백그라운드 동기화:** 서비스 워커(`sw.js`) 캐싱으로 인터넷 단절 시에도 완전 가동되며, 오프라인에서 기록한 데이터는 기기가 온라인 상태로 감지되면 Supabase 데이터베이스로 백그라운드 자동 병합(upsert)됩니다.
